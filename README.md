...
+     - UI 한글화: 게임 모드, 난이도, 버튼 등 UI의 주요 텍스트를 한글로 변경하여 사용자 편의성을 높였습니다.
+     - 보너스 획득 시 나타나는 화려한 텍스트 애니메이션 (`TETRIS!`, `T-Spin Double!`, `Combo!`)
+     - 게임 효과음: 블록 이동, 회전, 고정, 줄 제거, 게임 오버 등 다양한 상황에 맞는 사운드 재생
+     - 타격감 강화 효과:
+         - 스크린 쉐이크: 라인을 클리어할 때 화면이 흔들려 시각적인 쾌감을 더합니다.
+         - 블록 고정 파티클: 블록이 바닥에 고정되는 순간 파티클이 튀어 묵직한 타격감을 제공합니다.
+         - 라인 클리어 폭발: 라인을 지울 때마다 화려한 파티클 폭발 효과가 나타납니다.
+     - 게임 모드 설명: 게임 모드 선택 시, 해당 모드의 규칙과 목표를 설명하는 팝업 창이 나타나 사용자의 이해를 돕습니다.
+     - 게임 오버 시 화면 표시
  - 반응형 디자인 및 모바일 지원 (2025-07-23 구현, 2025-08-04 개선):
      - PC와 모바일 환경 모두에서 최적화된 화면 레이아웃
      - 모바일 기기를 위한 터치 컨트롤 인터페이스 제공
      - 모바일 컨트롤 개선: 조작 편의성을 높이기 위해 터치 컨트롤을 직관적인 십자 키(D-Pad) 레이아웃으로 변경
